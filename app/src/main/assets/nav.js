...
function bFallback(start,end,label){const miles=bMiles(start,end);const mins=bMode==='walk'?Math.max(1,Math.round(miles/3*60)):bMode==='bike'?Math.max(1,Math.round(miles/10*60)):Math.max(2,Math.round(miles/12*60));if(bRouteLine)map.removeLayer(bRouteLine);bRouteLine=L.polyline([[start.lat,start.lon],[end.lat,end.lon]],{weight:5,opacity:.9,color:'#35d0ff',dashArray:'8,10'}).addTo(map);map.fitBounds(bRouteLine.getBounds(),{padding:[45,45]});title.textContent=label;info.textContent=miles.toFixed(2)+' miles · about '+mins+' min';bRouteBox.innerHTML='<div style="font-weight:800;margin-bottom:6px">'+label+'</div><div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">1. Follow the blue dashed line toward the destination.</div><div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">2. For buildings, campuses, bus stops, and subway exits, exact road routes may not exist.</div><div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">3. Bus/subway mode needs live transit data; this version shows nearby guidance only.</div>';bRouteBox.style.display='block';}
async function bNavigate(){const start=bStartPoint();const end=bDestination();if(!end){title.textContent='Choose destination';info.textContent='Search or tap a place first.';return;}if(!start){title.textContent='No start location';info.textContent='Tap location first, or use Home as fallback.';return;}if(bMode==='walk'){bFallback(start,end,'Walking route');return;}if(bMode==='bike'){bFallback(start,end,'Bike route');return;}if(bMode==='bus'){bFallback(start,end,'Bus guidance');return;}if(bMode==='subway'){bFallback(start,end,'Subway guidance');return;}const label=bMode==='motorcycle'?'Motorcycle route':'Car route';title.textContent='Routing...';info.textContent='Calculating '+label.toLowerCase()+'.';try{const url='https://router.project-osrm.org/route/v1/driving/'+start.lon+','+start.lat+';'+end.lon+','+end.lat+'?overview=full&geometries=geojson&steps=true';const res=await fetch(url);const data=await res.json();if(!data.routes||!data.routes.length)throw new Error('no route');const route=data.routes[0];const points=route.geometry.coordinates.map(c=>[c[1],c[0]]);if(bRouteLine)map.removeLayer(bRouteLine);bRouteLine=L.polyline(points,{weight:6,opacity:.95,color:'#0b84ff'}).addTo(map);map.fitBounds(bRouteLine.getBounds(),{padding:[40,40]});const miles=(route.distance/1609.34).toFixed(1);const mins=Math.round(route.duration/60);title.textContent=label;info.textContent=miles+' miles · about '+mins+' min';const steps=route.legs[0].steps.slice(0,12);let html='<div style="font-weight:800;margin-bottom:6px">Steps</div>';for(let i=0;i<steps.length;i++){html+='<div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">'+(i+1)+'. '+bStepText(steps[i])+' · '+(steps[i].distance/1609.34).toFixed(1)+' mi</div>';}bRouteBox.innerHTML=html;bRouteBox.style.display='block';}catch(e){bFallback(start,end,bMode==='motorcycle'?'Motorcycle nearby guidance':'Nearby guidance');}}
