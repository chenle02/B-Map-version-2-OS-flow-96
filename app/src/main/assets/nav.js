...
function bVeryClose(start,end){if(bRouteLine){map.removeLayer(bRouteLine);bRouteLine=null;}const miles=bMiles(start,end);const feet=Math.round(miles*5280);title.textContent='Very close';info.textContent=feet+' ft away. No route needed.';bRouteBox.innerHTML='<div style="font-weight:800;margin-bottom:6px">Very close</div><div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">This point is already nearby.</div><div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">Move the pin to a road/path if you need exact navigation.</div>';bRouteBox.style.display='block';}
async function bNavigate(){const start=bStartPoint();const end=bDestination();if(!end){title.textContent='Choose destination';info.textContent='Search or tap a place first.';return;}if(!start){title.textContent='No start location';info.textContent='Tap location first, or manually set Home first.';return;}if(bMiles(start,end)<0.08){bVeryClose(start,end);return;}if(bMode==='bus'){bNoRoute('Bus route');return;}if(bMode==='subway'){bNoRoute('Subway route');return;}const label=bMode==='motorcycle'?'Motorcycle route':bMode==='walk'?'Walking route':bMode==='bike'?'Bike route':'Car route';title.textContent='Routing...';info.textContent='Calculating real route only.';const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),3500);try{const url='https://router.project-osrm.org/route/v1/driving/'+start.lon+','+start.lat+';'+end.lon+','+end.lat+'?overview=full&geometries=geojson&steps=true&radiuses=100;100';const res=await fetch(url,{signal:controller.signal});clearTimeout(timer);const data=await res.json();if(!data.routes||!data.routes.length)throw new Error('no route');const route=data.routes[0];const points=route.geometry.coordinates.map(c=>[c[1],c[0]]);if(bRouteLine)map.removeLayer(bRouteLine);bRouteLine=L.polyline(points,{weight:6,opacity:.95,color:'#0b84ff'}).addTo(map);map.fitBounds(bRouteLine.getBounds(),{padding:[40,40]});const miles=(route.distance/1609.34).toFixed(1);const mins=Math.round(route.duration/60);title.textContent=label;info.textContent=miles+' miles · about '+mins+' min';const routeSteps=route.legs[0].steps.slice(0,12);let html='<div style="font-weight:800;margin-bottom:6px">Route steps</div>';for(let i=0;i<routeSteps.length;i++){html+='<div style="padding:8px 0;border-top:1px solid rgba(255,255,255,.12);font-size:13px;color:#dcefff">'+(i+1)+'. '+bStepText(routeSteps[i])+' · '+(routeSteps[i].distance/1609.34).toFixed(1)+' mi</div>';}bRouteBox.innerHTML=html;bRouteBox.style.display='block';}catch(e){clearTimeout(timer);bNoRoute(label);}}
